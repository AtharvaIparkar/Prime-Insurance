...
export const ServiceDetailTemplate: React.FC<ServiceDetailProps> = ({ service }) => {
    return (
        <div className="py-24 container mx-auto px-4 mt-20 bg-white min-h-screen">
            <div className="max-w-4xl mx-auto">
                <Link href="/services" className="text-secondary font-bold mb-8 inline-flex items-center hover:text-primary uppercase tracking-widest text-lg transition-colors">
                ...
                <div className="flex flex-col md:flex-row md:items-center space-y-6 md:space-y-0 md:space-x-8 mb-16">
                    <div className="w-24 h-24 bg-primary/10 rounded-3xl flex items-center justify-center text-5xl shadow-inner">
                        {service.icon || '🏥'}
                    </div>
                    <div>
                        <p className="text-primary-dark font-black uppercase tracking-[0.4em] text-xs mb-2">Service Solution</p>
                        <h1 className="text-4xl md:text-6xl font-black text-hospital-slate tracking-tighter">{service.title}</h1>
                    </div>
                </div>

                <div className="prose prose-xl max-w-none text-gray-900 font-medium mb-16 leading-relaxed">
                    <p className="text-2xl leading-relaxed text-hospital-slate font-black mb-10 border-l-4 border-primary pl-8 italic">
                        {service.description}
                    </p>
                ...
                {service.bulletPoints && service.bulletPoints.length > 0 && (
                    <div className="bg-white p-12 rounded-[3.5rem] border border-gray-100 shadow-2xl mb-16 relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 blur-3xl -z-0" />
                        <h2 className="text-2xl font-black text-hospital-slate mb-10 uppercase tracking-tight relative z-10">Key Components & Benefits</h2>
                        <ul className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                            {service.bulletPoints.map((point, index) => (
                                <li key={index} className="flex items-start space-x-4">
                                    ...
                                    <span className="text-gray-900 font-black leading-tight text-lg tracking-tight">{point}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
